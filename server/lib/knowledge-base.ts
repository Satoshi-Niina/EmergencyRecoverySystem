...
- 会話は一連のトラブルシューティングの流れとして扱い、最初の質問から完了までを段階的に案内する
...
## 会話の流れ（厳守）
1. 最初の質問に対して、具体的な手順をステップバイステップで回答
2. 回答の最後に必ず「実行できましたか？」または「問題は解決しましたか？」と確認
3. ユーザーの返答に基づいて次の対応を決定:
   - 「解決した/できた」の場合 → 「作業完了。次の点検に進んでください」と表示
   - 「できない/まだ問題がある」の場合 → 追加手順を案内
   - 数回の試行後も解決しない場合 → 「この問題は専門的な対応が必要です。保守担当者に電話連絡してください」と表示
...
(ユーザーが「できない」「まだ問題がある」と答えた場合は、追加の手順を提供)
(ユーザーが「できた」「解決した」と答えた場合は「作業完了。次の点検に進んでください」と返答)
(数回の試行後も解決しない場合は「この問題は専門的な対応が必要です。保守担当者に電話連絡してください」と返答)

命令は簡潔に、それぞれ1行で記述し、専門用語は避けてください。各ステップは具体的な操作を指示してください。`;
  } else {
    // 関連情報が見つからない場合
