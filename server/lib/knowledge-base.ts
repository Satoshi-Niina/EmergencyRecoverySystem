...
## 回答方法（必ず守る）
- トラブルシューティング形式で、1問1答で回答する
- 各手順の前に番号を付けること（例：1. 2. 3.）
- 複数の手順がある場合は、必ず改行して1行につき1つの手順のみを記述する
- 質問に対する回答が終わったら、「対応完了しましたか？」と尋ねる
- もし対応が完了しておらず、さらに支援が必要な場合は「具体的に問題点を教えてください」と促す
- すべての対応が完了したら「対応完了。次の作業に進んでください」と表示する
- 対応不能な場合は「この問題は専門的な対応が必要です。保守担当者に電話連絡してください」と表示する
...
- エンジン：火災・熱傷危険`;
  ...
  if (relevantChunks.length > 0) {
    ...
    basePrompt += `\n上記の情報のみを参考にしながら、回答します。必ず【緊急復旧】のタイトルで始め、トラブルシューティング形式の1問1答で回答してください。各手順は必ず番号付きリストで、1行に1つの手順のみを記述してください。例えば：

1. エンジンキーを「OFF」に回す
2. 燃料バルブを閉める
3. エンジンカバーを開く

質問に対する回答が終わったら、「対応完了しましたか？」と尋ねてください。もし対応が完了しておらず、さらに支援が必要な場合は「具体的に問題点を教えてください」と促してください。対応不能の場合は「この問題は専門的な対応が必要です。保守担当者に電話連絡してください」と明記してください。`;
  } else {
    // 関連情報が見つからない場合
